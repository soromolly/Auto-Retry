...
// РАСШИРЕННАЯ ПРОВЕРКА: Ловим и Vertex, и Google AI Studio, и Gemini
...
async function handleGenerationEnded() {
    ...
    // Ждем чуть дольше, чтобы UI успел отрисовать всплывающее окно ошибки, если оно есть
    ...
    // 2. АНАЛИЗ ОШИБОК В ИНТЕРФЕЙСЕ (Ловим красные Toast-уведомления со сбоем 429 / Capacity)
    const toastError = document.querySelector('.toast-error, .toastr-error, .toast-message');
    let hasToastError = false;
    if (toastError) {
        const errorText = toastError.textContent.toLowerCase();
        if (errorText.includes('429') || errorText.includes('capacity') || errorText.includes('error') || errorText.includes('failed')) {
            hasToastError = true;
            console.log(`[${MODULE_NAME}] Обнаружено всплывающее окно ошибки: "${toastError.textContent}"`);
        }
    }

    // КРИТЕРИИ СБОЯ
    const isNoResponse = lastMessage.is_user === true; // Бот промолчал
    const isEmptyResponse = lastMessage.is_user === false && (!lastMessage.mes || lastMessage.mes.trim() === '');
    const isErrorInChatText = lastMessage.is_user === false && lastMessage.mes && (
        lastMessage.mes.includes('API Error') ||
        lastMessage.mes.includes('status code 400') ||
        lastMessage.mes.includes('status code 429')
    );

    // Если сработал текстовый триггер ИЛИ на экране висит плашка ошибки API
    if (hasToastError || isNoResponse || isEmptyResponse || isErrorInChatText) {
        ...
        // Если это была просто ошибка загрузки (висел toast), но чат чист, принудительно считаем за сбой
        retryCount++;
        isTimeoutActive = true;
        
        console.log(`[${MODULE_NAME}] Сбой API Gemini/Vertex. Запуск таймера на ${settings.interval} сек...`);
        
        // Закрываем окно ошибки, чтобы оно не мешало при следующем цикле
        if (toastError && typeof toastError.click === 'function') {
            toastError.click();
        }
            ...
            <div class="inline-drawer-content" style="display: none; padding: 10px 14px;">
                <div class="setup-block" style="display: flex; flex-direction: column; gap: 12px;">
                    ...
                    <div style="display: flex; flex-direction: column; gap: 6px;">
                        ...
                        <small style="opacity: 0.55; font-size: 0.8em; line-height: 1.2;">Скрипт подождет указанное время перед тем, как автоматически выполнить команду /retry.</small>
                    </div>
...
function init() {
    ...
    console.log(`[${MODULE_NAME}] Расширение Vertex/Gemini успешно обновлено.`);
}
